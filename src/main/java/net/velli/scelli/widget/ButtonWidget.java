...
public class ButtonWidget extends Widget<ButtonWidget> implements ClickableWidget {
    ...
    protected void render(DrawContext context, float mouseX, float mouseY, float delta) {
        ...
        if (hovered()) {
            context.fill(0, 0, Math.round(renderedWidth()), Math.round(renderedHeight()), 0x66333333);
            context.drawBorder(0, 0, Math.round(renderedWidth()), Math.round(renderedHeight()), 0xDDEEFFFF);
        } else context.fill(0, 0, Math.round(renderedWidth()), Math.round(renderedHeight()), 0x66000000);
        ...
        text.forEach(line -> {
            ...
            context.drawText(textRenderer, line, textX, textY.get(), 0xFFFFFFFF, true);
